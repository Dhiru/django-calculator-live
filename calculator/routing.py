...
from logs.consumers import connect_log, disconnect_log
...
channel_routing = [
    ...
    route("websocket.connect", connect_log, path=r'^/calculator/(?P<slug>[^/]+)/stream/$'),

    # Called when the client closes the socket
    route("websocket.disconnect", disconnect_log, path=r'^/calculator/(?P<slug>[^/]+)/stream/$'),
